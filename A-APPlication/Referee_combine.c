...
// 接收缓冲区
static uint8_t rx_buffer[REFEREE_RX_BUF_SIZE];
static uint16_t rx_data_len = 0;

// 发送缓冲区
static uint8_t tx_buffer[REFEREE_TX_BUF_SIZE];
...
{
    return (Referee_Calc_CRC8(data, len) == crc);
}
...
/**
 * @brief 获取当前机器人ID
 * @return 机器人ID（红方哨兵=7，蓝方哨兵=107）
 */
uint16_t Referee_Get_Robot_ID(void)
{
    return (g_team_color == TEAM_RED) ? ROBOT_ID_RED_SENTRY : ROBOT_ID_BLUE_SENTRY;
}
...
{
    // 使用HAL库发送数据
    HAL_UART_Transmit_DMA(&huart6, data, len);  // 根据实际配置修改
}

/**
 * @brief 裁判系统UART初始化
 * @note 需要在main.c中调用，配置USART为115200 8N1
 */
void Referee_UART_Init(void)
{
    // UART初始化已在MX_USART6_UART_Init()中完成
    // 这里可以添加额外的配置
    packet_seq = 0;
}
...
{
    // DMA初始化已在MX_DMA_Init()中完成
    // 启动DMA接收
    HAL_UART_Receive_DMA(&huart6, rx_buffer, REFEREE_RX_BUF_SIZE);
}
